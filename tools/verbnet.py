...
class VerbNetParser(object):
    ...
    def find_correct_subclass(self, vn_class, verb):
        if not vn_class:
            return False
        if vn_class in self.verb_classes_numerical_dict:
            ...
            if vn_class.split("-")[0] in self.verb_classes_numerical_dict:
                ...
                vn_class = vn_class.split(".")[0]
            elif vn_class + ".1" in self.verb_classes_numerical_dict:
                vn_class = vn_class + ".1"
            else:
                return False

        for subclass in [self.verb_classes_numerical_dict[vn_class]] + self.verb_classes_numerical_dict[vn_class].get_all_subclasses():
...
class ThematicRole(AbstractXML):
    ...
    def sel_restrictions(self, soup):
        ...
        if len(self.soup.contents) == 0:        # empty SELRESTRS
            ...
            children = [self.sel_restrictions(child) for child in soup.children]
            return [child for child in children if child is not None]
        elif len([child for child in soup.children]) == 3:
            return self.sel_restrictions(soup.find_all('SELRESTR')[0])
        else:
            return [self.sel_restrictions(child) for child in soup.find_all('SELRESTR')]
